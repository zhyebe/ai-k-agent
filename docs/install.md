...
普通监控轮次完成后立即开始下一轮；出现买卖提示时暂停，确认、取消或自动下单完成后恢复。监控轮次只分析最近 1 小时分钟数据。设置 `MONITOR_POLL_INTERVAL_MS` 可显式增加轮询间隔，范围为 1-120 秒；分析失败时使用最小退避重试。轮询是实时变化探测频率，不等于等待一根 K 线收盘；只有行情指纹变化才触发 AI 分析。
