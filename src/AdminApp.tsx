...
import { adminLogin, adminLogout, adminSession, assignAdminTask, createAdminUser, fetchAdminSummary, getApiBaseUrl, listAdminUsers, unassignAdminTask, updateAdminUser } from "./lib/api";
import type { AdminAccount, AdminSummary, AdminTaskSummary } from "./lib/api";
import type { EventItem } from "./types";

type AdminTab = "overview" | "accounts" | "audit";
type AdminUser = AdminAccount;
type AdminTask = AdminTaskSummary;
...
function taskStatusLabel(status: string) {
  return status === "MONITORING" ? "监控中" : status === "ANALYZING" ? "分析中" : status === "READY" ? "待启动" : status;
}
...
export default function AdminApp() {
  ...
  const tabs: Array<{ key: AdminTab; label: string; icon: typeof LayoutDashboard }> = [
    ...
    { key: "accounts", label: "账号与分配", icon: UserRound },
  ...
  return <div className="admin-shell">
    <aside className="admin-sidebar">
      ...
      <div className="admin-role"><ShieldCheck size={15} /><div><b>管理后台</b><span>账号、分配与日志</span></div></div>
    ...
    <main className="admin-main">
      ...
      <div className="admin-content">
        ...
        {tab === "accounts" && <AdminUsers users={users} tasks={summary.tasks} onChanged={setUsers} onToast={setToast} />}
...
function AdminUsers({ users, tasks, onChanged, onToast }: { users: AdminUser[]; tasks: AdminTask[]; onChanged: (users: AdminUser[]) => void; onToast: (message: string) => void }) {
  ...
  async function assign(userId: string, taskId: string) {
    if (!taskId) return;
    setBusyAction(`assign:${userId}`);
    try {
      const result = await assignAdminTask(userId, taskId);
      onChanged(users.map((user) => user.id === userId ? result.user : user));
      setSelectedTaskIds((current) => ({ ...current, [userId]: "" }));
      onToast("任务已分配");
    } catch (error) {
      onToast(`分配失败：${errorMessage(error)}`);
    } finally {
      setBusyAction(null);
    }
  }

  async function unassign(userId: string, taskId: string) {
    setBusyAction(`unassign:${userId}:${taskId}`);
    try {
      const result = await unassignAdminTask(userId, taskId);
      onChanged(users.map((user) => user.id === userId ? result.user : user));
      onToast("任务分配已取消");
    } catch (error) {
      onToast(`取消失败：${errorMessage(error)}`);
    } finally {
      setBusyAction(null);
    }
  }
  ...
  return <>
    <section className="admin-page-heading"><div><span className="eyebrow"><span className="eyebrow-line" />账号治理</span><h2>桌面账号</h2><p>创建登录账号、重置密码、启停访问，并分配可用任务。</p></div></section>
    ...
    <section className="admin-panel">
      ...
      {users.length ? users.map((user) => {
        const availableTasks = tasks.filter((task) => !user.assignedTaskIds.includes(task.id));
        const selectedTaskId = selectedTaskIds[user.id] || "";
        return <div className="admin-skill-row admin-user-row" key={user.id}>
          ...
          <div className="admin-skill-copy">
            ...
            <span>{accountStatusLabel(user.status)} · 已分配 {user.assignedTaskIds.length} 个任务</span>
            <div className="admin-assignment-list">{user.assignedTaskIds.length ? user.assignedTaskIds.map((taskId) => <span className="admin-assignment" key={taskId}><span>{tasks.find((task) => task.id === taskId)?.name || taskId}</span><button type="button" aria-label={`取消 ${taskId} 的任务分配`} title={`取消 ${taskId} 的任务分配`} onClick={() => unassign(user.id, taskId)} disabled={busyAction === `unassign:${user.id}:${taskId}`}><X size={12} /></button></span>) : <small>尚未分配任务</small>}</div>
          ...
          <div className="admin-user-actions">
            <select className="select-like" value={selectedTaskId} onChange={(event) => setSelectedTaskIds((current) => ({ ...current, [user.id]: event.target.value }))} aria-label={`为 ${user.username} 选择任务`} disabled={!availableTasks.length}>
              <option value="">{availableTasks.length ? "选择任务" : "暂无可分配任务"}</option>
              {availableTasks.map((task) => <option key={task.id} value={task.id}>{task.name} · {taskStatusLabel(task.status)}</option>)}
            </select>
            <button type="button" className="button button-small button-secondary" onClick={() => assign(user.id, selectedTaskId)} disabled={busyAction === `assign:${user.id}` || !selectedTaskId}><Plus size={13} />分配</button>
...
function AdminLogin({ onSignedIn }: { onSignedIn: () => void }) {
  ...
  return <div className="admin-login"><div className="admin-login-panel">
    <div className="brand-lockup"><div className="brand-mark" aria-hidden="true"><span /><span /><span /><span /></div><div><strong>axiom</strong><small>admin console</small></div></div>
    <div className="login-heading"><span className="eyebrow"><span className="eyebrow-line" />后台管理</span><h1>进入账号管理</h1><p>管理桌面端账号、任务分配和审计日志。</p></div>
...
function AdminOverview({ summary, users, apiAddress, onAccounts, onRefresh, onAudit }: { summary: AdminSummary; users: AdminUser[]; apiAddress: string; onAccounts: () => void; onRefresh: () => void | Promise<void>; onAudit: () => void }) {
  ...
  const assignedTaskIds = new Set(users.flatMap((user) => user.assignedTaskIds));
  const assignmentCount = users.reduce((total, user) => total + user.assignedTaskIds.length, 0);
  const unassignedTasks = Math.max(0, summary.tasks.length - assignedTaskIds.size);
  const recentUsers = users.slice(0, 6);
  const recentEvents = summary.events.slice(0, 5);
  return <>
    <section className="admin-page-heading"><div><span className="eyebrow"><span className="eyebrow-line" />平台运维</span><h2>账号与运行概览</h2><p>管理桌面端访问权限、任务分配与审计记录。</p></div><button type="button" className="button button-secondary" onClick={onRefresh}><RefreshCw size={15} />刷新状态</button></section>
    <div className="admin-metrics">
      ...
      <AdminMetric icon={<Activity size={17} />} label="任务分配" value={String(assignmentCount)} tone="blue" detail={`${assignedTaskIds.size} / ${summary.tasks.length} 个任务已覆盖`} />
      <AdminMetric icon={<ShieldCheck size={17} />} label="待分配任务" value={String(unassignedTasks)} tone="amber" detail="需要管理员处理" />
      <AdminMetric icon={<FileKey2 size={17} />} label="审计事件" value={String(summary.auditEvents)} tone="muted" detail="服务端记录" />
    </div>
    <div className="admin-two-col">
      <section className="admin-panel"><div className="admin-panel-head"><div><span className="panel-kicker"><UserRound size={14} />账号分配</span><h3>桌面端访问状态</h3></div><button type="button" className="text-button" onClick={onAccounts}>管理账号 <ArrowUpRight size={14} /></button></div>{recentUsers.length ? <div className="admin-account-summary-list">{recentUsers.map((user) => <div className="admin-account-summary-row" key={user.id}><span className="summary-avatar">{user.displayName.slice(0, 1).toUpperCase()}</span><div><strong>{user.displayName} · {user.username}</strong><span>{accountStatusLabel(user.status)} · {user.assignedTaskIds.length} 个任务</span></div><span className={`account-status ${user.status === "ACTIVE" ? "active" : "disabled"}`}>{accountStatusLabel(user.status)}</span></div>)}</div> : <EmptyState label="还没有桌面账号" />}</section>
      <section className="admin-panel"><div className="admin-panel-head"><div><span className="panel-kicker"><Database size={14} />服务状态</span><h3>管理服务健康</h3></div><span className={`service-health ${summary.persistence.available ? "ready" : "offline"}`}><span />{summary.persistence.available ? "正常" : "不可用"}</span></div><div className="admin-service-list"><div className="admin-service-row"><span className="service-icon green"><ShieldCheck size={15} /></span><div><strong>管理 API</strong><span>{apiAddress}</span></div><b>在线</b></div><div className="admin-service-row"><span className="service-icon blue"><Database size={15} /></span><div><strong>持久化数据库</strong><span>{summary.persistence.detail || summary.persistence.mode}</span></div><b className={summary.persistence.available ? "ok" : "warn"}>{summary.persistence.available ? "已连接" : "待连接"}</b></div><div className="admin-service-row"><span className="service-icon muted"><Activity size={15} /></span><div><strong>任务目录</strong><span>{summary.totalTasks} 个任务 · {summary.activeTasks} 个运行中</span></div><b>只读</b></div></div><div className="health-note"><ShieldCheck size={14} /><span>后台只维护账号、分配和日志，不读取或修改用户端连接器、模型密钥和 Skills 配置。</span></div></section>
    </div>
...
function AdminAudit({ events }: { events: EventItem[] }) {
  ...
  return <>
    <section className="admin-page-heading"><div><span className="eyebrow"><span className="eyebrow-line" />可追溯性</span><h2>审计日志</h2><p>记录账号登录、账号状态、任务分配与服务运行事件。</p></div><button type="button" className="button button-secondary" onClick={exportAudit}><FileText size={15} />导出日志</button></section>
