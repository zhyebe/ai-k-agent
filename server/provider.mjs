...
function normalizeDecision(value) {
  ...
  const boardAssessments = (Array.isArray(value?.board_assessments) ? value.board_assessments : [])
    .map((item) => ({
      ...
      profitProbability: Math.min(1, Math.max(0, Number(item?.profit_probability ?? item?.profitProbability ?? item?.confidence) || 0)),
  ...
  return {
    ...
    confidence,
    profitProbability: Math.min(1, Math.max(0, Number(value?.profit_probability ?? value?.profitProbability ?? value?.win_probability ?? confidence) || 0)),
    decisionTtlSec: ttl,
