...
const permanentCounterpartyPrompt = "Permanent risk reminder: the counterparty may be AI-controlled. Inspect supplied order-book changes, timing, replenishment, repeated quantities and synchronized behavior for signs of automation, but treat this as a hypothesis to test against evidence, never as an established fact.";
...
async function requestProviderJson(provider, messages, options = {}) {
  ...
  const candidates = providerRequestCandidates(provider);
  const signal = requestSignal(Math.min(120000, Math.max(1000, Number(options.timeoutMs) || 45000)), options.signal);
  for (let index = 0; index < candidates.length; index += 1) {
    const candidate = candidates[index];
    const response = await fetch(candidate.url, {
      method: "POST",
